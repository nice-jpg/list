module.exports = function () {
  ...
  (function ($) {
     ...
     * - ''innerScroll''     {Boolean}              (可选, 默认值:false)是否是内滚，若内滚，则不绑定eventName事件，用户需在外部绑定相应的事件，可调$.fn.lazyload.detect去检测图片是否出现在container中
     ...
     * @example $('.lazy-load').lazyload();
     * $('.lazy-load').lazyload().on('error', function (e) {
    ...
    $.fn.lazyload = function (opts) {
      ...
      if ($.isFunction($.fn.lazyload.detect)) {
        //若是增加图片，则处理placeHolder
        _addPlsHolder();
      ...
      function _load(div) {
        ...
        if (!isImg) {
          $.each($div.get(0).attributes, function () {
            //若不是img作为容器，则将属性名中含有data-的均增加到图片上
