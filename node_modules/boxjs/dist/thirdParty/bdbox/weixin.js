module.exports = function () {
  ...
  /**
   * 分享到微信朋友圈
   * @memberOf Bdbox.weixin
   * @function shareToTimeline
   * @param    {Object}    data       待分享的信息
   * @param    {String}    data.appId      公众平台的appId（服务号可用）
   * @param    {String}    data.imgUrl     图片地址
   * @param    {String}    data.link       链接地址
   * @param    {String}    data.desc       描述
   * @param    {String}    data.title      分享的标题
   *
   * @param    {Object}    callbacks  相关回调方法
   * @param    {Boolean}   callbacks.async                   ready方法是否需要异步执行，默认false
   * @param    {Function}  callbacks.ready(argv)             就绪状态
   * @param    {Function}  callbacks.dataLoaded(data)        数据加载完成后调用，async为true时有用，也可以为空
   * @param    {Function}  callbacks.cancel(resp)    取消
   * @param    {Function}  callbacks.fail(resp)      失败
   * @param    {Function}  callbacks.confirm(resp)   成功
   * @param    {Function}  callbacks.all(resp)       无论成功失败都会执行的回调
   */
   ...
   * @param    {Object}    callbacks  相关回调方法
   * @param    {Boolean}   callbacks.async                   ready方法是否需要异步执行，默认false
   * @param    {Function}  callbacks.ready(argv,shareTo)             就绪状态
   * @param    {Function}  callbacks.dataLoaded(data)        数据加载完成后调用，async为true时有用，也可以为空
   * @param    {Function}  callbacks.cancel(resp,shareTo)    取消
   * @param    {Function}  callbacks.fail(resp,shareTo)      失败
   * @param    {Function}  callbacks.confirm(resp,shareTo)   成功
   * @param    {Function}  callbacks.all(resp,shareTo)       无论成功失败都会执行的回调
   */
   ...
   * @param {function} callback 返回如下几种类型：
   * ```
   ...
   *
   * // 当页面加载完毕后执行，使用方法：
   * Bdbox.weixin.ready(function(Api){
   *     // 从这里只用Api即是WeixinApi
   * });
   *
   */
