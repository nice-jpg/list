module.exports = function () {
   ...
   *
   *
   * mid {Number} 上报对应的类型;如果mid === -1，window.onerror专用，其他：1=>js错误，2：网络错误，3：图片请求错误 ，4：样式错误
   * level {Number} 上报对应的级别 1 => debug（调试日志）; 2 = > info（流水日志）; 4 => error（错误日志）（默认值） ; 8 => fail（致命错误日志）
   */


  var getVersion = require('../utils/getVersion');

  var BROWSER_DATA = ['boxv=' + getVersion(), 'isbox=' + ($.$isBox() ? 1 : 0)];
  var levels = {
    'debug': 1,
    'info': 2,
    'error': 4,
  ...
  var BJ_REPORT = function (global) {
    ...
    var _config = {
      id: 0,
      url: "http://boxrd.baidu.com/badjs",
      combo: 0,
      ext: {},
      level: 4,
      // 1-debug 2-info 4-error 8-fail
      ignore: [],
      random: 1,
      delay: 0,
      submit: null
    ...
    var orgError = global.onerror; // rewrite window.onerror
    ...
    var report = {
      ...
      init: function (config) {
        ...
        if (id) {
          _config.report = _config.url + "?id=" + id + "&from=" + encodeURIComponent(location.href) + "&" + BROWSER_DATA.join('&') + "&ext=" + JSON.stringify(_config.ext) + "&";
        } else {
