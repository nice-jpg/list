module.exports = function () {
  ...
  (function (win, $) {
    ...
    var browser = {
      ...
      isT5: /T5\//.test(win.navigator.userAgent)
    }; // 触摸事件

    var events = ['touchstart', 'touchmove', 'touchend']; // 检测
    ...
    if (browser.ie11) events = ['pointerdown', 'pointermove', 'pointerup']; // 触摸赋值
    ...
    sS.prototype.init = function (options) {
      ...
      me._index = me.opts.index; // 轮播数量

      me._liLength = me.opts.li.length;
      me.isScrolling; // 回调

      me.opts.firstCallback(me._index, me._liLength, me.$el); // 如果轮播小于等于1个，跳出
      ...
      if (me.opts.continuousScroll) {
        me.opts.ul.prepend(me.opts.li.last().clone().css('margin-left', '-100%')).append(me.opts.li.first().clone());
      } // 懒加载图片
      ...
      if (me.opts.lazyLoad) {
        ...
        if (me.opts.continuousScroll) {
          ...
          fnLazyLoad(me, me._index + 2); // 如果是第一屏

          if (me._index == 0) {
            fnLazyLoad(me, me._liLength); // 如果是最后一屏
      ...
      } // T5内核预热启动3d加速，并且动画元素要有Text类型节点


      if (browser.isT5) {
        me.$el.before($('<div>').css({
          '-webkit-transform': 'translate3d(0,0,0)',
          'transform': 'translate3d(0,0,0)'
        }));
        me.$el.prepend($('<div>&nbsp;</div>').css('height', 0));
      } // 调用轮播
      ...
      function fnGetSlideDistance() {
        ...
        me._slideDistance = me.opts.axisX ? me.opts.ul.width() : me.opts.ul.height(); // 定位

        fnTransition(me, me.opts.ul, 0);
        fnTranslate(me, me.opts.ul, -me._slideDistance * me._index); // 使用box布局只在ul元素做translate，不在li元素。
    ...
    function fnTranslate(me, dom, distance) {
      var result = me.opts.axisX ? distance + 'px,0' : '0,' + distance + 'px'; // T5内核translate3d图片模糊

      if (browser.isT5) {
        result = 'translate(' + result + ')';
      } else {
        result = 'translate3d(' + result + ',0)';
      }
    ...
    function fnTouchstart(e, me) {
      ...
      me._moveDistance = me._moveDistanceIE = 0; // 按下时的坐标
    ...
    function fnTouchmove(e, me) {
      ...
      me.allowSlideClick = false; // 触摸时的坐标

      me._curX = support.touch ? e.touches[0].pageX : e.pageX || e.clientX;
      me._curY = support.touch ? e.touches[0].pageY : e.pageY || e.clientY; // 触摸时的距离

      me._moveX = me._curX - me._startX;
      me._moveY = me._curY - me._startY; // 优化触摸禁止事件
      ...
      if (me.isScrolling) {
        if (e.preventDefault) e.preventDefault();else e.returnValue = false; // 触摸时跟手
    ...
    function fnSlide(me, go, num) {
      ...
      if (typeof go === 'number') {
        ...
        if (me.opts.lazyLoad) {
          if (me.opts.continuousScroll) {
            fnLazyLoad(me, me._index + 2); // 滑到最后一屏

            if (me._index + 1 == me._liLength) {
              fnLazyLoad(me, 1); // 最后一屏，继续往后滑动
        ...
        if (me.opts.lazyLoad) {
          if (me.opts.continuousScroll) {
            fnLazyLoad(me, me._index); // 滑到第一屏

            if (me._index == 0) {
              fnLazyLoad(me, me._liLength); // 第一屏，继续往前滑动
            } else if (me._index < 0) {
