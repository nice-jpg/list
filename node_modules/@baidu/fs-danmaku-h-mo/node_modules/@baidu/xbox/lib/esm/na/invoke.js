 ...
 * @return Promise 对象
 */
export function invokeP(action, params = {}, schemaHead = getSchemaHead()) {
    if (!isBox() && !isMatrix()) {
    ...
    return new Promise((resolve, reject) => {
        ...
        params.callback = function (result) {
            ...
            const { status, data: successData } = data;
            if (+status === 0) {
                // 成功回调
                resolve(successData);
 ...
 */
export function xRegister(range = {}, action, params = {}, schemaHead = getSchemaHead()) {
    if (!isBox() && !isMatrix()) {
        return Promise.reject(NOT_IN_BOX);
    }
    ...
    if (opt.v) {
        ...
        if (versionCompare(version, startV) >= 0 && (!endV || (endV && versionCompare(version, endV) <= 0))) {
            ...
            if (others && getType(others) === 'object') {
                args = Object.assign({ params: json2params(params) }, others);
            }
            else {
