...
var Distribute = function(opts) {
    ...
    if (url && url !== '') {
        this.androidIntent = 'intent://' + url.replace(/^http[s]?:\/\//, '') + '#Intent;scheme=http;action=com.baidu.searchbox.action.VIEW;category=android.intent.category.DEFAULT;end';
        // this.androidSchema = '';
    ...
    this.fail = function() {
        ...
        if ($.isFunction(opts[$.os.name + 'FailCallback'])) {
            var back = opts[$.os.name + 'FailCallback']()
                //如果返回是false,则不往下执行，即不跳转下载
