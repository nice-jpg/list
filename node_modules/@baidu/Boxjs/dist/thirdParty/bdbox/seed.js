...
;
    ...
    $._options = {
        ...
        comboPattern: '<!--[COMBO_SEVER_URL]-->??%s', // '/path/to/combo-service/%s' or function (ids) { return url; }
        combo: 1,
...
;
    ...
    $.load = function (url, options) {
        ...
        function onerror(e) {
            ...
            if (options.onerror) options.onerror.call($, e);
            else throw e;
        }
    ...
    rproto.push = function () {
        ...
        for (var i = 0, l = args.length; i < l; i++) {
            var id = $.alias(args[i]);
            //将andorid/invokeApp缩写，转成common:bdbox/android/invokeApp,
            ...
            var deps = conf.deps;
            deps && deps.length && that.push.apply(that, deps);
            //如果找到pkg，直接加载pkg文件，pkg包含所有依赖
            var pkg = conf.pkg;
            var isPkg = false;
            if (pkg && PKG[pkg]) {
                // console.log(id);
