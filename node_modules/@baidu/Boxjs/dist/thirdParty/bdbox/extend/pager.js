...
!function ($) {
    ...
    $.extend(Pager.prototype,{
        ...
        builder : function () {
            ...
            if (pn > 1) {
                list.push({ type : 'prev', index : pn - 1, enabled : true });
            } else {
                list.push({ type : 'prev', enabled : false });
            }
            ...
            if (count > 12) {
                ...
                if (pn < 8) {
                    for (var i = 1 ; i <= Math.min(count, 9) ; ++ i) {
                        if (pn == i) {
                            list.push({ type : 'index', index : i, selected : true });
                        } else {
                            list.push({ type : 'index', index : i, selected : false });
                        }
                    }

                    list.push({ type : 'last', index : count });

                } else if (pn + 8 > count) {

                    list.push({ type : 'first', index : 1 });
                    ...
                    list.push({ type : 'index', index : pn, selected : true });
            ...
            if (pn < count) {
                list.push({ type : 'next', index : pn + 1, enabled : true });
            } else {
                list.push({ type : 'next', enabled : false });
            }

            if (count > 12) {
                list.push({ type : 'to', enabled : true });
            }
        },
        ...
        ToString : function () {
            var tpl = {
                ...
                to : function (e) {
                    return '';
                    // return '<li class="pager-to">到<input type="text" class="pager-text" />页<span class="btn btn-normal"><span><button type="button">确定</button></span></span></li>';
                },
    ...
    $.fn.pager = function (options) {
        ...
        if (typeof(options) == 'object' || !options) {
            ...
            var p = $.extend({ core : this }, options);
