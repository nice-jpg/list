...
var IP_SERVICE_URL = 'http://api.map.baidu.com/location/ip?ak=syDXGG8s0DjYkYSnCj7jQlUq&coor=bd09ll&callback=?';
...
var LBS = {
    ...
    getFromLightApp: function(options) {
        ...
        window[sucName] = function(data) {
            if (data.result === 0) {
                ...
                options.onFail({
                    error: data.result, //lightapp返回失败
                    from: 'lightapp'
                });
            }
        };
    ...
    getFromIP: function(options) {
        ...
        serviceHandlerCache[handlerCacheName].push(options);
        var cb = function(data) {
            var handler, cbname = 'onSuccess';

            if (data && data.status === 0 && data.content && data.content.point) {
                var content = data.content;
                var point = content.point;
                var addressDetail = content.address_detail;
                var position = new Position(point.x, point.y, 0, addressDetail.city_code, Date.now(), 'ip', content.address, addressDetail);
                //存入cache
                cache.ip = position;
                data = position;
...
return LBS;
//http://bigbox.searchbox.baidu.com/bigbox?action=loc&type=locinfo&format=json&prod=searchbox&resource_name=other&sec=1405923180&authcode=f20eab&cip=223.104.12.250&cuid=9A34AC1B4F1BF41D21E1C2B9D6F8A7A99D44C44FFFNLMELLSGE&log_id=3006043974&data={'location':'102.646135%2C25.053514%2C--460-02'}
