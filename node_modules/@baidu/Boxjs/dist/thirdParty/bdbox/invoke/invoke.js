...
var IS_IN_BOX = $.$isBox();
// var HOST_REG = /(?:baidu|hao123).com/i;
...
Invoke.prototype = {
    ...
    showUpdate: function() {
        ...
        if ($.os.ios) {
            ...
            if (template) {
                ...
                // $node.list.download.addEventListener('click', function() {
                //     //触发download事件
                //     self.fire('download');
                // }, false);
    ...
    cmd: function(action, args, callback) {
        ...
        if (IS_IN_BOX || $.os.ios) {
            ...
            args = $.isString(args) ? args : JSON.stringify(args);
            //这里真恶心！因为moplus乱七八糟的接口！！传入给moplus数据有的需要encode，有的不要！
            //没有规律，只能单个处理 %>_<%
            switch (action) {
                case 'card.add':
...
var Template = '<div data-node="box-openapp" style="margin:0;padding:0;-webkit-text-size-adjust:100%;">' +
    '<div data-node="mask" style="z-index:99999990;opacity:.5;background-color:#000;position: absolute;top:0;left:0;height:100%;width:100%;font-size:15px;"></div>' +
    '<div data-node="layer" style="z-index:99999999;width:280px;height:154px;overflow:hidden;position:absolute;top:10px;left:50%;margin-left:-140px;-webkit-border-radius:2px;border-radius:2px;background:#fff;">' +
    '<a data-node="close" href="javascript:void(0);" style="position: absolute; display: block; width: 48px; height: 48px; top: 0px; right: 0px;background-image: url(http://m.baidu.com/static/search/inapp/lib/popup_sprite_png8.png?v=20130424); background-size:14px 80px; -webkit-background-size:14px 80px; background-position:26px -58px; background-repeat:no-repeat no-repeat;"></a>' +
    '<p style="font-size:16px;color:#262626;margin:22px 16px 10px 16px;text-align:left;min-height:1px;">使用手机百度</p><p style="color:#2a8de8;font-size:16px;margin:10px 16px;text-align:left;">添加后下次访问更方便</p><p style="margin:10px 16px;color:#999;text-align:left;">请下载最新版</p><div style="margin:16px 16px 21px 16px;line-height:30px;text-align:left;font-size:13px;"><a href="http://dl.ops.baidu.com/baidusearch_AndroidPhone_1001608s.apk" style="display:inline-block;-webkit-box-sizing:border-box;height:30px;width:150px;text-decoration:none;text-align:center;border:1px solid #7da9f2;background:#9ac4ff;color:#fff;"><span style="padding-left:20px;background:url(http://m.baidu.com/static/search/inapp/lib/popup_sprite_png8.png?v=20130424) left -15px no-repeat;background-size:14px 80px;-webkit-background-size:14px 80px;">立即下载</span></a>' +
    '</div>' +
    '</div>' +
    '</div>';
