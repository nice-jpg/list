/**
 * @file 判断方法
 * @module is
 * @author wangyongqing <wangyongqing01@baidu.com>
 */

import {isType} from './type';

export interface KeyValue {
    [key: string]: unknown;
}

/**
 * 判断是否为 null 或者 undefined
 *
 * @param obj 判断的对象
 * @return 返回判断结果
 * @example
 * isNil(null) // true
 * isNil(undefined) // true
 * isNil('') // false
 */
export function isNil(obj: unknown) {
    return obj === undefined || obj === null;
}

/**
 * 判断是否为 undefined
 *
 * @param obj 判断的对象
 * @return 返回判断结果
 * @example
 * isUndefined(null) // false
 * isUndefined(undefined) // true
 * isUndefined('') // false
 */
export function isUndefined(obj: unknown) {
    return obj === undefined;
}

/**
 * 判断是否是 window 对象
 *
 * @param obj 判断的对象
 * @return 返回判断结果
 * @example
 * isWindow(window) // true
 * isWindow(self) // true
 * isWindow('') // false
 */
export function isWindow(obj: unknown) {
    return !isNil(obj) && obj === (obj as any).window;
}

/**
 * 判断是否为普通对象
 *
 * @param obj 需要判断的对象
 * @return 返回判断结果
 * @example
 * isPlainObject({}) // true
 * isPlainObject(window) // false
 */
export function isPlainObject(obj: unknown) {
    if (isType(obj, 'Object')) {
        return Object.getPrototypeOf(obj) === null || Object === (obj as any).constructor;
    }

    return false;
}

/**
 * 是否是 function 类型
 *
 * @param obj 要判断的对象
 * @return 返回判断结果
 * @example
 * isFunction(new Function()) // true
 * isFunction('') // false
 */
export function isFunction(obj: unknown) {
    return isType(obj, 'Function');
}

/**
 * 是否是 string 类型
 *
 * @param obj 要判断的对象
 * @return 返回判断结果
 * @example
 * isString([]) // false
 * isString('array') // true
 */
export function isString(obj: unknown) {
    return isType(obj, 'String');
}

/**
 * 是否是 array 类型
 *
 * @param obj 要判断的对象
 * @return 返回判断结果
 * @example
 * isArray([]) // true
 * isArray('array') // false
 */
export function isArray(obj: unknown) {
    return isType(obj, 'Array');
}

/**
 * 是否是一个 Number 类型
 *
 * @param obj 要判断的对象
 * @return 返回判断结果
 * @example
 * isNumber(1) // true
 * isNumber('1') // false
 */
export function isNumber(obj: unknown) {
    return isType(obj, 'Number');
}

/**
 * 判断是否为布尔类型
 *
 * @param bool 要判断的对象
 * @return 返回判断结果
 * @example
 * isBoolean(false) // true
 * isBoolean(/a/) // false
 */
export function isBoolean(bool: unknown) {
    return bool === true || bool === false;
}

/**
 * 判断参数是否是空字符串
 *
 * @param subject 被判断的字符串
 * @return 返回判断结果
 * @example
 * isEmpty(''); // => true
 * isEmpty('  '); // => false
 * isEmpty('World'); // => false
 */
export function isEmpty(subject: string) {
    return String(subject).length === 0;
}

/**
 * 判断参数是否是空格或者由空格组成
 *
 * @param subject 被判断的字符串
 * @return 判断结果
 * @example
 * isBlank('');// => true
 * isBlank('  ');// => true
 * isBlank('World');// => false
 */
export function isBlank(subject: string) {
    return String(subject).trim().length === 0;
}

/**
 * 判断一个对象是不是一个空对象
 *
 * @param obj 待判断的对象
 * @return 判断结果
 * @example
 * isEmptyObject({}) // true
 */
export function isEmptyObject(obj: KeyValue) {
    return Object.keys(obj).length === 0 && obj.constructor === Object;
}

const regJSON = /^\{[\s\S]*\}$|^\[[\s\S]*\]$/;

/**
 * 判断一个对象是不是 JSONString
 *
 * @param obj 待判断的对象
 * @return 判断结果
 * @example
 * isJSON({}) // true
 */
export function isJSON(value: unknown) {
    if (typeof value !== 'string') {
        return false;
    }
    if (!regJSON.test(value)) {
        return false;
    }
    try {
        JSON.parse(value);
    }
    catch (err) {
        return false;
    }
    return true;
}
