...
type MonitorBeforeMethod = (data: KeyValue) => false | KeyValue;
...
export class Monitor {
    private _mdata: KeyValue;
    private readonly _url: string;
    private readonly _default: KeyValue;
    private readonly _data: KeyValue;
    private readonly _before?: MonitorBeforeMethod;
    ...
    _send(data: KeyValue, force = false) {
        if (!isPlainObject(data)) {
            throw typeError('data', 'object', data);
        }

        let img = new Image(1, 1) as any;
        const imgName = `_xanybox_mt_${guid()}`;
        // window[imgName] = img;
        ...
        if (!force && before && isFunction(before)) {
        ...
        const url = `${this._url}?${isString(data) ? data : json2query(data, true)}`;
