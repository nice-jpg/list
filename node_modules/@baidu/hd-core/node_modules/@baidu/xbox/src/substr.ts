...
const hasUnicode = (val: string) => reHasUnicode.test(val);
const unicodeToArray = (val: string) => val.match(reUnicode) || [];
const asciiToArray = (val: string) => val.split('');
 ...
 */
export function str2array(val: string, {hasEmoji = false, emojiMaxLen = 8, emojiData = []}: EmojiOptions = {}) {
    if (!hasEmoji) {
...
type RestArgs = [(number | EmojiOptions)?, EmojiOptions?];
 ...
 */
export function substr(str: string, start: number, ...rest: RestArgs) {
    const hasLength = typeof rest[0] === 'number';
    const data = hasLength ? str2array(str, rest[1]) : str2array(str, rest[0] as EmojiOptions);
    const ret = hasLength ? data.splice(start, rest[0] as number) : data.splice(start);
    return ret.join('');
}
