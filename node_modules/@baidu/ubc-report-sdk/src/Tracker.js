...
Tracker.prototype.regestTimingReportor = function (fields = []) {
    ...
    let ifHeartbeatTime = this.get('ifHeartbeatTime');
    let opt = {};
    if (fields && isObject(fields[1])) {
        opt = {
            type: fields[0],
            ...fields[1]
        }
    ...
    let isExist = find(timingTasks, task => task.eventName === fields[0]);
    // 注册时长点时，加入starttime
    opt.starttime = Date.now();
    // 如果不存在就先注册
    if (!isExist) {
        let timingIntevalId = 0;
        if (ifHeartbeatTime) {
            // // 发送第一次时长上报信息
            this.send({
                type: 'timing',
    ...
    let data = {
        ...
        actiondata: {
            ...
            content: {
                ...
                page: model.page || window.location.hostname + window.location.pathname,
                // 资源来源
                source: model.source || getQueryParams('idfrom') || '',
                // 业务来源
                from: model.from,
                // 外部传入-事件名称
                // 外部传入-场景信息
...
function generateDurationData(model, opt) {
    ...
    if (!model.ifHeartbeatTime) {
        data.actiondata.endtime = Date.now();
        data.actiondata.content.duration = (data.actiondata.endtime - data.actiondata.starttime) / 1000;
    }
    // v1 有，v2没有的参数
    if (model.version === '1.0') {
        // ab分组信息
...
function setBasicConfig(model, opt) {
    ...
    // 时长打点是否走心跳模式
    if (opt.config && isBoolean(opt.config.ifHeartbeatTime)) {
        model.ifHeartbeatTime = opt.config.ifHeartbeatTime;
    } else {
        model.ifHeartbeatTime = true;
    }
    ...
    // 如果 version = 2.0，则增加参数 v，增加appname的默认值
    if (model.version) {
        model.queryParams.v = model.version;
        model.queryParams.appname = model.queryParams.appname || 'baiduboxapp';
    }
    ...
    // 默认的渠道信息
    model.source = opt.config && opt.config.source || '';
    ...
    // 现在在取值的时候赋予默认值，而不是在初始化的时候赋予默认值
    model.page = opt.config && opt.config.page || '';
    ...
    // 宿主信息
    model.apptype = getAppByName().appid;
}
